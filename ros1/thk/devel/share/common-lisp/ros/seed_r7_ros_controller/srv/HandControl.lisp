...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<HandControl-request>)))
  ...
  (cl:format cl:nil "uint8 position  # right or left hand~%uint8 POSITION_RIGHT = 0~%uint8 POSITION_LEFT = 1~%~%string script   # script number (string value for print)~%string SCRIPT_GRASP = grasp~%string SCRIPT_RELEASE = release~%string SCRIPT_CANCEL = cancel~%~%uint8  current  # grasping current setting[%]~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'HandControl-request)))
  "Returns full string definition for message of type 'HandControl-request"
  (cl:format cl:nil "uint8 position  # right or left hand~%uint8 POSITION_RIGHT = 0~%uint8 POSITION_LEFT = 1~%~%string script   # script number (string value for print)~%string SCRIPT_GRASP = grasp~%string SCRIPT_RELEASE = release~%string SCRIPT_CANCEL = cancel~%~%uint8  current  # grasping current setting[%]~%~%~%"))
